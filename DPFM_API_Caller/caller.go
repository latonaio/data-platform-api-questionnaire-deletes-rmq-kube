...
import (
	"context"
	dpfm_api_input_reader "data-platform-api-questionnaire-deletes-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-questionnaire-deletes-rmq-kube/DPFM_API_Output_Formatter"
...
func (c *DPFMAPICaller) headerDelete(
	...
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "QuestionnaireHeader", "runtime_session_id": sessionID})
	if err != nil {
	...
	for i := range *items {
		...
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": (*items)[i], "function": "QuestionnaireItem", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			log.Error("%+v", err)
			return nil, nil
		...
		if !checkResult(res) {
			output.SQLUpdateResult = getBoolPtr(false)
			output.SQLUpdateError = "Questionnaire Item Data cannot delete"
			return nil, nil
		}
	}
...
func (c *DPFMAPICaller) itemDelete(
	...
	for _, v := range input.Header.Item {
		data := dpfm_api_output_formatter.Item{
			...
			QuestionnaireItem:		v.QuestionnaireItem,
			IsMarkedForDeletion:	v.IsMarkedForDeletion,
		}
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": data, "function": "QuestionnaireItem", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			log.Error("%+v", err)
			return nil
